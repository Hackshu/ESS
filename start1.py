...
def builder():
    server_host = input(MESSAGE_INPUT + "Server host (where ESS will connect to): ")
    server_port = int(input(MESSAGE_INPUT + "Server port: "))
    program_directory = input(MESSAGE_INPUT + "Where should ESS live? "
...
def main():
    ...
    parser.add_argument("--no-banner", help="prevents the ESS banner from being displayed", action="store_true")
